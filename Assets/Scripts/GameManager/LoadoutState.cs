...
public class LoadoutState : AState
{
    ...
    public IEnumerator PopulateCharacters()
    {
        ...
        if (!m_IsLoadingCharacter)
        {
            ...
            while (newChar == null)
            {
                CharactersSet c = CharacterDatabase.GetCharacter(PlayerData.instance.characters[PlayerData.instance.usedCharacter]);

                if (c != null)
                {
                    m_OwnedAccesories.Clear();
                    for (int i = 0; i < c.mainCharacter.accessories.Length; ++i)
                    {
						// Check which accessories we own.
                        string compoundName = c.characterName + ":" + c.mainCharacter.accessories[i].accessoryName;
    ...
    void SetupAccessory()
    {
        CharactersSet charactersSet = m_Character.GetComponent<CharactersSet>();
        Character c = charactersSet.mainCharacter;
    ...
    void SetupCharactersSets()
    {
        var set = m_Character.GetComponent<CharactersSet>();
    }
