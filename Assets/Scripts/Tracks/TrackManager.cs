...
{
    static int s_switchZone = Animator.StringToHash("switchZone");
    ...
    //delay before remove
    protected List<TrackSegment> m_bufferSegments = new List<TrackSegment>();
    ...
    protected const int k_DesiredSegmentCount = 8;
    protected const float k_SegmentRemovalDistance = -130f;
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            player = op.Result.GetComponent<CharactersSet>();

            var lastCharacter = player.mainCharacter;
            lastCharacter.SetupAccesory(PlayerData.instance.usedAccessory);

            characterController.character = lastCharacter;
    ...
    public CharactersSet player;
    public float segmentOffset = 6f;
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    public IEnumerator SpawnNewSegment()
    {
        ...
        if (!m_IsTutorial)
        {
            if (m_CurrentThemeData.zones[m_CurrentZone].length < m_CurrentZoneDistance)
            {
                ...
                // PlayerData.instance.usedCharacter = (PlayerData.instance.usedCharacter + 1) % PlayerData.instance.characters.Count;
                // var op = Addressables.InstantiateAsync(PlayerData.instance.characters[PlayerData.instance.usedCharacter],
                //Vector3.zero,
                //Quaternion.identity);
                // yield return op;
        ...
        //if (zoneChanged)
        //{
        //    var op = Addressables.InstantiateAsync("ZoneCollider",
        //       currentExitPoint,
        //       currentExitRotation);
        //    yield return op;
        //    op.Result.GetComponent<ZoneCollider>().switchCharacter = true;
        //    op.Result.transform.SetParent(newSegment.transform);

        //    var op1 = Addressables.InstantiateAsync("ZoneCollider",
        //       currentExitPoint,
        //       currentExitRotation);
        //    yield return op1;
        //    op1.Result.GetComponent<ZoneCollider>().startGlith = true;
        //    op1.Result.GetComponent<BoxCollider>().size = new Vector3(20,20, 2);
        //    op1.Result.transform.SetParent(newSegment.transform);
        ...
        Vector3 center = (newSegment.transform.position - entryPoint);
        Vector3 pos = currentExitPoint + center;
        ...
        {
            var op = Addressables.InstantiateAsync("ZoneCollider",
               Vector3.zero - center + new Vector3(0, 0, 0.5f),
               Quaternion.identity);
            yield return op;

            ZoneCollider zoneCollider1 = op.Result.GetComponent<ZoneCollider>();
            zoneCollider1.switchCharacter = true;
            zoneCollider1.newZone = m_CurrentThemeData.zones[m_CurrentZone].zoneIndex;

            op.Result.transform.SetParent(newSegment.transform, false);
            op.Result.transform.GetChild(0).gameObject.SetActive(false);

            var op1 = Addressables.InstantiateAsync("ZoneCollider",
               Vector3.zero - center + new Vector3(0, 0, -3f),
               Quaternion.identity);
            yield return op1;
            ZoneCollider zoneCollider = op1.Result.GetComponent<ZoneCollider>();
            zoneCollider.startGlith = true;
            //op1.Result.GetComponent<BoxCollider>().size = new Vector3(20, 20, 6);
            op1.Result.transform.localScale = new Vector3(20, 20, 6);
            op1.Result.transform.SetParent(newSegment.transform, false);
        }
    ...
    {
        //var cameraAminator = Camera.main.GetComponent<Animator>();
        //if (cameraAminator)
        //{
        //    cameraAminator.Play(s_switchZone);
        //}
        var clitch = Camera.main.GetComponent<AnalogProcessGlitch>();
        clitch.On();
        //clitch.splitOn = true;
        characterController.characterCollider.SetInvincibleExplicit(true);
    }

    public void ChangeCharacter(int zoneIndex)
    {
        //        if (zone.zoneIndex != 0)
        {
            var character = player.zoneCharacters.Find(x => x.zoneIndex == zoneIndex);
            if (character == null || zoneIndex == 0)
            {
                character = player.mainCharacter;
            }
